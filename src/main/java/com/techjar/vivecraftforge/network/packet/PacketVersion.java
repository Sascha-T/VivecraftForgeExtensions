...
public class PacketVersion implements IPacket {
	...
	public void decode(final FriendlyByteBuf buffer) {
		...
		buffer.readBytes(bytes); // i do not question it [insert chess king sacrifice meme here]
		...
		if (!message.contains("NONVR")) {
			...
			context.get().enqueueWork(() -> {
				...
				if (Config.enableJoinMessages.get() && !Config.joinMessageVR.get().isEmpty())
                    // FriendlyByteBuf
					player.getServer().getPlayerList().broadcastMessage(new TextComponent(String.format(Config.joinMessageVR.get(), player.getDisplayName())), ChatType.SYSTEM, Util.NIL_UUID);
			});
		} else {
			...
			context.get().enqueueWork(() -> {
				...
				if (Config.enableJoinMessages.get() && !Config.joinMessageNonVR.get().isEmpty())
					player.getServer().getPlayerList().broadcastMessage(new TextComponent(String.format(Config.joinMessageNonVR.get(), player.getDisplayName())), ChatType.SYSTEM, Util.NIL_UUID);
			});
		}
	}
}
