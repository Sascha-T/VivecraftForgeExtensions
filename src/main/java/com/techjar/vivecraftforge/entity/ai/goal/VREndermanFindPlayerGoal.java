...
public class VREndermanFindPlayerGoal extends EnderMan.EndermanLookForPlayerGoal {
	...
	private final TargetingConditions lineOfSightPredicate = TargetingConditions.forCombat();
		...
		if (this.pendingTarget != null) {
			...
			if (this.target != null && !this.enderman.isPassenger()) {
				if (Util.shouldEndermanAttackVRPlayer(this.enderman, (Player) this.target)) {
					...
					this.teleportTime = 0;
				} else if (this.pendingTarget.distanceToSqr(this.enderman) > 256.0D && this.teleportTime++ >= 30 && this.enderman.teleportTowards(this.target)) {
					this.teleportTime = 0;
				}
			}
