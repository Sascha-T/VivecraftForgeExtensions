	...
	public boolean canUse() {
		...
		if (target instanceof Player) {
			...
			if (data != null && !data.seated)
				return this.creeper.getSwellDir() > 0 || this.creeper.getPosition(0f).distanceTo(target.getPosition(0f)) < Config.creeperSwellDistance.get() * Config.creeperSwellDistance.get();
		}
		return super.canUse();
	}
}
